...
public class PostTechStackRepositoryTest {
    ...
    public void findPostTechStacks(){
        ...
        Post firstPost = setupPost(40);
        PageRequest pageRequest = PageRequest.of(0, 20, Sort.by("createdDate"));
        PostSearchRequestDto postSearchRequestDto = new PostSearchRequestDto(PostStatus.RECRUITING.name(), Arrays.asList(tag.getName()));

        //when
        List<PostTechStack> postTechStacks = postTechStackRepository.findPostTechStacks(postSearchRequestDto, pageRequest);

        //then
        assertThat(postTechStacks.size()).isEqualTo(20);
        assertThat(postTechStacks.get(0).getId()).isEqualTo(firstPost.getId());
    }
    ...
    public void findRecommendedPosts(){
        ...
        PostRecommendRequestDto postRecommendRequestDto = new PostRecommendRequestDto(Arrays.asList(tag.getName()));

        //when
        List<PostTechStack> recommendedPosts = postTechStackRepository.findRecommendedPosts(postRecommendRequestDto, user.getId(), post.getId());
